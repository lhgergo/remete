# TODO: ugyanazon fajta kimeneteket adja az interface_gdrive, mint az interface_file, hogy a többi függvény tudjon velük mit kezdeni

out <- PackIn(3+2) %>% SendAway(interface = "interface_gdrive")
GetBack(out)
...
taskpack <- PackIn(RunNetMHCpan(alleles = c("HLA-A02:01", "HLA-A03:01"), peptides = peps,
                   ...
                   objects = c("peps"),
                   libraries = c("emhace.tools")) %>% SendAway(interface = "interface_gdrive")

GetBack(taskpack, interface = "interface_gdrive")
